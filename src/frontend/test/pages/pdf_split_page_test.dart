...
import 'package:free_ocr_frontend/pages/pdf_split_page.dart';
import 'package:free_ocr_frontend/pages/pdf_split_progress_page.dart';
...
void main() {
  ...
  testWidgets('PdfSplitPage renders dropzone, header, and AdSense banner initially', (WidgetTester tester) async {
  ...
  testWidgets('PdfSplitProgressPage renders document overview, mode options, and split action', (WidgetTester tester) async {
    tester.view.physicalSize = const Size(1280, 1000);
    tester.view.devicePixelRatio = 1.0;
    addTearDown(tester.view.resetPhysicalSize);
    ...
    await tester.pumpWidget(
      MaterialApp(
        home: PdfSplitProgressPage(file: mockFile),
      ),
    );
    await tester.pumpAndSettle();
  ...
  testWidgets('PdfSplitProgressPage toggles modes and validates ranges', (WidgetTester tester) async {
    tester.view.physicalSize = const Size(1280, 1000);
    tester.view.devicePixelRatio = 1.0;
    addTearDown(tester.view.resetPhysicalSize);
    ...
    expect(find.textContaining('Extract all 10 pages into individual single-page PDFs'), findsOneWidget);
    ...
    expect(find.textContaining('exceeds document page count (10)'), findsOneWidget);
