...
import 'package:free_ocr_frontend/pages/pdf_watermark_page.dart';
import 'package:free_ocr_frontend/pages/pdf_watermark_progress_page.dart';
...
void main() {
  ...
  testWidgets('PdfWatermarkPage renders dropzone, header, and AdSense banner initially', (WidgetTester tester) async {
    ...
    expect(
      find.textContaining('Stamp custom text or transparent image watermarks across your document'),
      findsOneWidget,
    );
  ...
  testWidgets('PdfWatermarkProgressPage renders overview, watermark controls, and live preview box', (WidgetTester tester) async {
    ...
    await tester.pumpWidget(
      MaterialApp(
        home: PdfWatermarkProgressPage(file: mockFile),
      ),
    );
    await tester.pumpAndSettle();
    ...
    expect(find.widgetWithText(ElevatedButton, 'Apply Watermark'), findsOneWidget);
  });

  testWidgets('Typing custom text updates live preview text', (WidgetTester tester) async {
    tester.view.physicalSize = const Size(1280, 1200);
    tester.view.devicePixelRatio = 1.0;
    addTearDown(tester.view.resetPhysicalSize);
    ...
    final customTextField = find.byKey(const Key('watermark_custom_text_field'));
  ...
  testWidgets('Switching watermark type switches to image logo controls', (WidgetTester tester) async {
    tester.view.physicalSize = const Size(1280, 1200);
    tester.view.devicePixelRatio = 1.0;
    addTearDown(tester.view.resetPhysicalSize);
