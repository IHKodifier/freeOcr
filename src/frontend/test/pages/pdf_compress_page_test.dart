...
import 'package:free_ocr_frontend/pages/pdf_compress_page.dart';
import 'package:free_ocr_frontend/pages/pdf_compress_progress_page.dart';
...
void main() {
  ...
  testWidgets('PdfCompressPage renders dropzone, header, and AdSense banner initially', (WidgetTester tester) async {
    ...
    expect(
      find.textContaining('Reduce PDF file size while preserving high visual resolution'),
      findsOneWidget,
    );

    // Verify Dropzone prompt & dynamic limits notice
    expect(find.textContaining('Drop PDF file here to compress'), findsOneWidget);
  ...
  testWidgets('PdfCompressProgressPage renders overview, 3 preset cards, and AdSense banner', (WidgetTester tester) async {
    ...
    await tester.pumpWidget(
      MaterialApp(
        home: PdfCompressProgressPage(file: mockFile),
      ),
    );
    await tester.pumpAndSettle();
  ...
  testWidgets('Tapping compression level presets updates selection state', (WidgetTester tester) async {
    tester.view.physicalSize = const Size(1280, 1200);
    tester.view.devicePixelRatio = 1.0;
    addTearDown(tester.view.resetPhysicalSize);
