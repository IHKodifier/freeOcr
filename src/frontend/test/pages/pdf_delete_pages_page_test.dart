...
import 'package:free_ocr_frontend/pages/pdf_delete_pages_page.dart';
import 'package:free_ocr_frontend/pages/pdf_delete_pages_progress_page.dart';
...
void main() {
  ...
  testWidgets('PdfDeletePagesPage renders dropzone, header, and AdSense banner initially', (WidgetTester tester) async {
  ...
  testWidgets('PdfDeletePagesProgressPage renders document overview, page cards, status bar, and delete button', (WidgetTester tester) async {
    ...
    await tester.pumpWidget(
      MaterialApp(
        home: PdfDeletePagesProgressPage(file: mockFile),
      ),
    );
    await tester.pumpAndSettle();
    ...
    expect(find.textContaining('0 of 3 pages marked to remove'), findsOneWidget);

    // Verify primary action button
    expect(find.widgetWithText(ElevatedButton, 'Delete Pages & Generate PDF'), findsOneWidget);
  });

  testWidgets('PdfDeletePagesProgressPage toggling card marks for deletion and updates status', (WidgetTester tester) async {
    tester.view.physicalSize = const Size(1280, 1200);
    tester.view.devicePixelRatio = 1.0;
    addTearDown(tester.view.resetPhysicalSize);
    ...
    expect(find.textContaining('1 of 3 pages marked to remove'), findsOneWidget);
  ...
  testWidgets('PdfDeletePagesProgressPage selecting all pages disables delete button with guard warning', (WidgetTester tester) async {
    tester.view.physicalSize = const Size(1280, 1200);
    tester.view.devicePixelRatio = 1.0;
    addTearDown(tester.view.resetPhysicalSize);
    ...
    expect(find.textContaining('A PDF must retain at least one page'), findsOneWidget);
