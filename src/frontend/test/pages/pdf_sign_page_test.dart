...
import 'package:free_ocr_frontend/pages/pdf_sign_page.dart';
import 'package:free_ocr_frontend/pages/pdf_sign_progress_page.dart';
...
void main() {
  ...
  testWidgets('PdfSignPage renders dropzone, header, and AdSense banner initially', (WidgetTester tester) async {
    ...
    expect(
      find.textContaining('Draw, type, or upload verifiable digital signatures to sign PDF documents'),
      findsOneWidget,
    );
  ...
  testWidgets('PdfSignProgressPage renders overview, page selector, and signature controls', (WidgetTester tester) async {
    ...
    await tester.pumpWidget(
      MaterialApp(
        home: PdfSignProgressPage(file: mockFile),
      ),
    );
    await tester.pumpAndSettle();
    ...
    expect(find.byKey(const Key('open_signature_dialog_btn')), findsOneWidget);
    expect(find.byKey(const Key('sign_placement_canvas')), findsOneWidget);

    // Verify Apply Signature button
    expect(find.widgetWithText(ElevatedButton, 'Apply Signature'), findsOneWidget);
  });

  testWidgets('Signature modal opens and switches between Draw, Type, and Upload tabs', (WidgetTester tester) async {
  ...
  testWidgets('Page selector increments and decrements target signing page', (WidgetTester tester) async {
    tester.view.physicalSize = const Size(1280, 2000);
    tester.view.devicePixelRatio = 1.0;
    addTearDown(tester.view.resetPhysicalSize);
