...
import 'package:free_ocr_frontend/pages/pdf_redact_page.dart';
import 'package:free_ocr_frontend/pages/pdf_redact_progress_page.dart';
...
void main() {
  ...
  testWidgets('PdfRedactPage renders header, dropzone, security card, and AdSense banner initially', (WidgetTester tester) async {
    ...
    expect(
      find.textContaining('Permanently black out sensitive text, data, and confidential areas'),
      findsOneWidget,
    );
  ...
  testWidgets('PdfRedactProgressPage renders overview, search & redact bar, and security guarantee shield', (WidgetTester tester) async {
    ...
    await tester.pumpWidget(
      MaterialApp(
        home: PdfRedactProgressPage(file: mockFile),
      ),
    );
    await tester.pumpAndSettle();
    ...
    expect(find.textContaining('True Cryptographic Sanitization'), findsOneWidget);

    // Verify Action button
    expect(find.text('Sanitize & Redact PDF'), findsOneWidget);
  });

  testWidgets('Entering search phrase updates input controller and enables action', (WidgetTester tester) async {
    tester.view.physicalSize = const Size(1280, 1200);
    tester.view.devicePixelRatio = 1.0;
    addTearDown(tester.view.resetPhysicalSize);
  ...
  testWidgets('Toggling case sensitivity switch updates state correctly', (WidgetTester tester) async {
    tester.view.physicalSize = const Size(1280, 1200);
    tester.view.devicePixelRatio = 1.0;
    addTearDown(tester.view.resetPhysicalSize);
