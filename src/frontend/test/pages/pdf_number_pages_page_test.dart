...
import 'package:free_ocr_frontend/pages/pdf_number_pages_page.dart';
import 'package:free_ocr_frontend/pages/pdf_number_pages_progress_page.dart';
...
void main() {
  ...
  testWidgets('PdfNumberPagesPage renders dropzone, header, and AdSense banner initially', (WidgetTester tester) async {
    ...
    expect(
      find.textContaining('Add clean, customizable page numbers to your PDF documents'),
      findsOneWidget,
    );
  ...
  testWidgets('PdfNumberPagesProgressPage renders overview, position matrix, format options, and live preview', (WidgetTester tester) async {
    ...
    await tester.pumpWidget(
      MaterialApp(
        home: PdfNumberPagesProgressPage(file: mockFile),
      ),
    );
    await tester.pumpAndSettle();
    ...
    expect(find.widgetWithText(ElevatedButton, 'Apply Page Numbers'), findsOneWidget);
  });

  testWidgets('Position grid selection updates active position and preview state', (WidgetTester tester) async {
    tester.view.physicalSize = const Size(1280, 1200);
    tester.view.devicePixelRatio = 1.0;
    addTearDown(tester.view.resetPhysicalSize);
  ...
  testWidgets('Format selector updates live preview text', (WidgetTester tester) async {
    tester.view.physicalSize = const Size(1280, 1200);
    tester.view.devicePixelRatio = 1.0;
    addTearDown(tester.view.resetPhysicalSize);
