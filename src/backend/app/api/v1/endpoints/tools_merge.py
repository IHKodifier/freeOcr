...
async def merge_pdf_files(
    ...
    """
    Merges 2 to 50 uploaded PDF documents into a single document in sequential order.
    ...
    try:
        for idx, file in enumerate(files):
            safe_name = f"input_{idx:03d}.pdf"
            dest_path = Path(temp_dir) / safe_name

            # Stream contents to temp storage while tracking size
            size = 0
