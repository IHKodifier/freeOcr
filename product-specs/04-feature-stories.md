...
| ID | Story | Acceptance Criteria | MVP? | Priority |
|----|-------|---------------------|------|---------|
| **US-301** | As the website publisher, I want display ads to refresh according to a runtime configurable timer (`monetization.ad_rotation_interval_seconds`, default 35s fetched dynamically from `GET /api/v1/config`) so that ad rotation frequency can be updated instantly from `app_limits_config.json` without rebuilding or redeploying the Flutter app. | - [ ] Top leaderboard banner rendered above fold on Landing & Download pages.<br>- [ ] Fetches `ad_rotation_interval_seconds` dynamically from `GET /api/v1/config` on app boot.<br>- [ ] Automated timer script refreshes ad slot when page tab is active. | ✅ | P0 |
