...
| **Hosting & Deployment** | **Firebase Hosting (Frontend Static CDN) + GCP Cloud Run (Backend API & OCR Workers)** | Decoupled hybrid architecture: Homepage (`/`) retains instant Flutter Web Hero Dropzone with `index.html` semantic SEO fallback tags for 100/100 search/AdSense crawling. Articles/Blogs (`/blog/*`) are authored in Markdown (`.md`) and compiled to static SEO HTML via `scripts/build_seo_pages.py` (zero HTML/React authoring required). GCP Cloud Run API scales to `min-instances = 0` for $0 idle cost. | Free-tier Firebase Hosting CDN + GCP Cloud Run scale-to-zero |
| **Monetization Runtime Config** | **Single Canonical Config (`app_limits_config.json`) via `GET /api/v1/config`** | Ad rotation interval (`ad_rotation_interval_seconds`, default 35s), rewarded ad durations, and limit boosts are fetched dynamically from backend config at runtime. Config changes take effect instantly without rebuilding or redeploying the Flutter Web app. | Zero-rebuild runtime config updates |
